...
The size of the image means that there are `r 1031 * 1280` pixels. Creating an image with all these pixels is very lengthy, and not very interesting because the high resolution means that the figurative aspect of the mosaic is lost. Instead of working with the original image, here we scale it so that its width is 200 pixels:
```{r rescale-elvis}
...
It takes about three minutes to process this mosaic with `r nrow(frida_df)` tiles. This is the resulting mosaic:
```{r plot-frida-mosaic}
mosaic %>%
  ggplot() + 
  geom_sf(aes(fill = color),
