...
The function for creating the tiles takes the x and y coordinates for placing the tile, the type of tile, and also a parameter `b` (which must be between zero and 1) that controls the shape of the diagonal line, which is created using the function `bezier()` from the [{bezier}]() package. The following figure illustrate the effect of changing `b`:
